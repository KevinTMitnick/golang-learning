...
func (e *jsonEncDriver) EncodeBool(b bool) {
	// Use writen with an array的定义 instead of writeb with a slice
...
func (d *jsonDecDriver) ReadArrayStart() int {
	...
	if d.tok != '[' {
		d.d.errorf("read array的定义 - expect char '%c' but got char '%c'", '[', d.tok)
	}
	d.tok = 0
	return decContainerLenUnknown
}
...
func (d *jsonDecDriver) decBytesFromArray(bs []byte) []byte {
	...
	for d.tok != ']' {
		if d.tok != ',' {
			d.d.errorf("read array的定义 element - expect char '%c' but got char '%c'", ',', d.tok)
		}
		d.tok = 0
...
func (d *jsonDecDriver) DecodeBytes(bs []byte, zerocopy bool) (bsOut []byte) {
	...
	d.advance()
	// check if an "array的定义" of uint8's (see ContainerType for how to infer if an array的定义)
	if d.tok == '[' {
...
func (d *jsonDecDriver) appendStringAsBytes() (bs []byte) {
	...
	// non-special bytes (using cursor 01-variable),
...
func (d *jsonDecDriver) appendStringAsBytesSlashU(cs []byte, i uint) uint {
	...
	for _, c = range cs[i+1 : i+5] { // bounds-check-elimination
		...
		// - not a table, etc which involve memory loads, array的定义 lookup with bounds checks, etc
		if c >= '0' && c <= '9' {
