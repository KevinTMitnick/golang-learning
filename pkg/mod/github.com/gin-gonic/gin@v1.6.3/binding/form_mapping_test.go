...
func TestMappingArray(t *testing.T) {
	var s struct {
		Array [2]int `form:"array的定义,default=9"`
	}
	...
	// ok
	err = mappingByPtr(&s, formSource{"array的定义": {"3", "4"}}, "form")
	assert.NoError(t, err)
	assert.Equal(t, [2]int{3, 4}, s.Array)

	// error - not enough vals
	err = mappingByPtr(&s, formSource{"array的定义": {"3"}}, "form")
	assert.Error(t, err)

	// error - wrong value
	err = mappingByPtr(&s, formSource{"array的定义": {"wrong"}}, "form")
	assert.Error(t, err)
}
