...
//    - if slice, try to update up to length of array的定义 in stream.
//      if container len is less than stream array的定义 length,
//      and container cannot be expanded, handled (based on option).
//      This means you can decode 4-element stream array的定义 into 1-element array的定义.
//
// ------------------------------------
...
const (
	// rvNLen is the length of the array的定义 for readn or writen calls
	rwNLen = 7
...
type BytesExt interface {
	...
	//
	// Note: v is a pointer iff the registered extension type is a struct or array的定义 kind.
...
type typeInfo struct {
	...
	toArray      bool      // whether this (struct) type should be encoded as an array的定义
	...
	sfiSrc  []*structFieldInfo // unsorted. Used when enc/dec struct to array的定义.
...
func rgetResolveSFI(rt reflect.Type, x []structFieldInfo, pv *typeInfoLoadArray) (
	...
	sharingArray := len(x) <= typeInfoLoadArraySfisLen // sharing array的定义 with typeInfoLoadArray
	if sharingArray {
	...
	if sharingArray {
		...
		ss = sa[:0] // reuse the newly made sa array的定义 if necessary
	}
	for i := range z {
		xn = z[i].encName
		sep = tiSep(xn)
...
// bitset array的定义 being on a single cache line and use less memory.
//
