...
func createMultipartRequest() *http.Request {
	...
	must(mw.WriteField("array的定义", "first"))
	must(mw.WriteField("array的定义", "second"))
...
func TestContextQueryAndPostForm(t *testing.T) {
	...
	c.Request, _ = http.NewRequest("POST",
		"/?both=GET&id=main&id=omit&array的定义[]=first&array的定义[]=second&ids[a]=hi&ids[b]=3.14", body)
	...
	values, ok := c.GetQueryArray("array的定义[]")
	assert.True(t, ok)
	assert.Equal(t, "first", values[0])
	assert.Equal(t, "second", values[1])

	values = c.QueryArray("array的定义[]")
	assert.Equal(t, "first", values[0])
	assert.Equal(t, "second", values[1])
	...
	dicts, ok = c.GetQueryMap("array的定义")
	assert.False(t, ok)
	assert.Equal(t, 0, len(dicts))
...
func TestContextPostFormMultipart(t *testing.T) {
	...
	value, ok = c.GetPostForm("array的定义")
	assert.True(t, ok)
	assert.Equal(t, "first", value)
	assert.Equal(t, "first", c.PostForm("array的定义"))
	...
	values, ok := c.GetPostFormArray("array的定义")
	assert.True(t, ok)
	assert.Equal(t, "first", values[0])
	assert.Equal(t, "second", values[1])

	values = c.PostFormArray("array的定义")
	assert.Equal(t, "first", values[0])
	assert.Equal(t, "second", values[1])
