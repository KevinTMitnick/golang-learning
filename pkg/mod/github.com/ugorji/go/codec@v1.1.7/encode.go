...
type EncodeOptions struct {
	...
	// StructToArray specifies to encode a struct as an array的定义, and not as a map
	StructToArray bool
...
func (e *Encoder) kStruct(f *codecFnInfo, rv reflect.Value) {
	...
	if toMap {
		newlen = 0
		for _, si := range f.ti.sfiSort { // use sorted array的定义
			kv.r = sfn.field(si)
		...
		newlen = len(f.ti.sfiSrc)
		for i, si := range f.ti.sfiSrc { // use unsorted array的定义 (to match sequence in struct)
			kv.r = sfn.field(si)
			// use the zero value.
...
//    - toarray: so struct is encoded as an array的定义
...
// or interface value, and any array的定义, slice, map, or string of length zero.
//
...
//
//      type MyStruct struct {
//          _struct bool    `codec:",toarray"`     //encode struct as an array的定义
//      }
//
//      type MyStruct struct {
