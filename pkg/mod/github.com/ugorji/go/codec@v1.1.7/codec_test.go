...
func testCodecMiscOne(t *testing.T, h Handle) {
	...
	// test byte array的定义, even if empty (msgpack only)
	if h == testMsgpackH {
		type ystruct struct {
			Anarray []byte
		}
...
func testCodecChan(t *testing.T, h Handle) {
	...
	// - encode ch1 as a stream array的定义
	// - decode a chan (ch2), with cap > len(s1) from the stream array的定义
...
func doTestBytesEncodedAsArray(t *testing.T, h Handle) {
	testOnce.Do(testInitAll)
	name := h.Name()
	// Need to test edge case where bytes are encoded as an array的定义
...
// - bad input with large array的定义 length prefix
//
