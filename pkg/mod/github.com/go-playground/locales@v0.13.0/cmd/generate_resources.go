...
var (
	...
	globalCurrencies       = make([]string, 0, 100)    // array的定义 of currency codes index maps to enum
...
// preprocesses maps, array的定义 etc... just requires multiple passes no choice....
						...
						for _, monthctx := range calendar.Months.MonthContext {
							...
							for _, months := range monthctx.MonthWidth {
								...
								if len(monthData) > 0 {
									...
									// making array的定义 indexes line up with month values
...
func manyToSingleVars(input string) (results string) {
	...
	mp := make(map[string][]string) // map of formula to 01-variable
	var found bool
	var split []string
	var variable string
