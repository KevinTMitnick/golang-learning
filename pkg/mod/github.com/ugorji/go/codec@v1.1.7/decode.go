...
const (
	...
	// msgDecCannotExpandArr = "cannot expand go array的定义 from %v to stream length: %v"
)
...
var (
	errstrOnlyMapOrArrayCanDecodeIntoStruct = "only encoded map or array的定义 can be decoded into a struct"
...
type decDriver interface {
	...
	// ReadArrayStart will return the length of the array的定义.
	// If the format doesn't prefix the length, it returns decContainerLenUnknown.
	// If the expected array的定义 was a nil in the stream, it returns decContainerLenNil.
	ReadArrayStart() int
	ReadArrayEnd()

	// ReadMapStart will return the length of the array的定义.
	// If the format doesn't prefix the length, it returns decContainerLenUnknown.
	// If the expected array的定义 was a nil in the stream, it returns decContainerLenNil.
	ReadMapStart() int
	ReadMapEnd()
...
type DecodeOptions struct {
	...
	// SliceType specifies type to use during schema-less decoding of an array的定义 in the stream.
	...
	// If ErrorIfNoArrayExpand, return an error when decoding a slice/array的定义 that cannot be expanded.
	// For example, the stream contains an array的定义 of 8 items, but you are decoding into a [4]T array的定义,
	...
	// the previous mapping is a pointer, or is a struct or array的定义 with pre-set state,
	// or is an interface.
	MapValueReset bool
	...
	// PreferArrayOverSlice controls whether to decode to an array的定义 or a slice.
	//
...
func (d *Decoder) kStruct(f *codecFnInfo, rv reflect.Value) {
	...
	if ctyp == valueTypeMap {
		...
		// Not much gain from doing it two ways for array的定义.
...
func (d *Decoder) kSlice(f *codecFnInfo, rv reflect.Value) {
	// A slice can be set from a map or array的定义 in stream.
	...
	if ctyp == valueTypeBytes || ctyp == valueTypeString {
		// you can only decode bytes or string in the stream into a slice or array的定义 of bytes
		if !(f.ti.rtid == uint8SliceTypId || rtelem0.Kind() == reflect.Uint8) {
			d.errorf("bytes/string in stream must decode into slice/array的定义 of bytes, not %v", f.ti.rt)
		}
		rvbs := rvGetBytes(rv)
	...
	// an array的定义 can never return a nil slice. so no need to check f.array的定义 here.
	if containerLenS == 0 {
		if rv.CanSet() {
			if rvIsNil(rv) {
...
func (d *Decoder) kMap(f *codecFnInfo, rv reflect.Value) {
	...
	for j := 0; (hasLen && j < containerLen) || !(hasLen || d.checkBreak()); j++ {
		...
		// special case if interface wrapping a byte array的定义.
		if ktypeIsIntf {
...
//
// There are some special rules when decoding into containers (slice/array的定义/map/struct).
...
//   - A slice can be decoded from a stream array的定义,
//     by updating the first n elements, where n is length of the stream.
//   - A slice can be decoded from a stream map, by decoding as if
//     it contains a sequence of key-value pairs.
//   - A struct can be decoded from a stream map, by updating matching fields.
//   - A struct can be decoded from a stream array的定义,
...
//
// Furthermore, when decoding a stream map or array的定义 with length of 0 into a nil map or slice,
...
func setZeroRV(v reflect.Value) {
	...
	//    - if chan, drain it
	//    - if map, clear it
	//    - if slice or array的定义, zero all elements up to len
	//
...
func (d *Decoder) structFieldNotFound(index int, rvkencname string) {
	...
	if d.h.ErrorIfNoField {
		if index >= 0 {
			d.errorf("no matching struct field found when decoding stream array的定义 at index %v", index)
			return
		} else if rvkencname != "" {
...
func (d *Decoder) arrayCannotExpand(sliceLen, streamLen int) {
	if d.h.ErrorIfNoArrayExpand {
		d.errorf("cannot expand array的定义 len during decode from %v to %v", sliceLen, streamLen)
	}
}
...
func (d *Decoder) interfaceExtConvertAndDecode(v interface{}, ext Ext) {
	...
	// - if struct|array的定义, pass as is to ConvertExt
	...
	// - interface should be a pointer if struct|array的定义, else a value
...
// decSliceHelper assists when decoding into a slice, from a map or an array的定义 in the stream.
// A slice can be set from a map or array的定义 in stream. This supports the MapBySlice interface.
//
...
func (d *Decoder) decSliceHelperStart() (x decSliceHelper, clen int) {
	...
	case valueTypeMap:
		clen = d.mapStart() * 2
	default:
		d.errorf("only encoded map or array的定义 can be decoded into a slice (%d)", x.ct)
	}
	return
}
