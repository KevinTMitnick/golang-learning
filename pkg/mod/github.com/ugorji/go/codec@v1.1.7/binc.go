...
func (d *bincDecDriver) ReadArrayStart() (length int) {
	...
	if d.vd != bincVdArray {
		d.d.errorf("array的定义 - %s %x-%x/%s", msgBadDesc, d.vd, d.vs, bincdesc(d.vd, d.vs))
		return
	}
	length = d.decLen()
	d.bdRead = false
...
func (d *bincDecDriver) decodeExtV(verifyTag bool, tag byte) (xtag byte, xbs []byte) {
	if d.vd == bincVdCustomExt {
		...
		d.d.errorf("ext - expecting extensions or byte array的定义 - %s %x-%x/%s",
			msgBadDesc, d.vd, d.vs, bincdesc(d.vd, d.vs))
		return
	}
	d.bdRead = false
...
//   is not encoded in the byte array的定义 explicitly*.
//
