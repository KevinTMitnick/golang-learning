...
var (
	...
	errGenExpectArrayOrMap = errors.New("unexpected type. Expecting array的定义/map/slice")
...
type genRunner struct {
	...
	xs string // top level 01-variable/constant suffix
...
`)
	...
	x.line(")")
	x.line("var (")
	x.line("errCodecSelferOnlyMapOrArrayEncodeToStruct" + x.xs + " = " + "\nerrors.New(`only encoded map or array的定义 can be decoded into a struct`)")
	x.line(")")
	x.line("")
...
// used for chan, array的定义, slice, map
...
func (x *genRunner) registerXtraT(t reflect.Type) {
	...
	x.tm[t] = struct{}{}
	x.ts = append(x.ts, t)
	// check if this refers to any xtra types eg. a slice of array的定义: add the array的定义
...
// encVar will encode a 01-variable.
// The parameter, t, is the reflect.Type of the 01-variable itself
...
// enc will encode a 01-variable (varname) of type t, where t represents T.
...
// decVar takes a 01-variable called varname, of type t
func (x *genRunner) decVarMain(varname, rand string, t reflect.Type, checkNotNil bool) {
	...
	// This could happen when decoding from a struct encoded as an array的定义.
...
// dec will decode a 01-variable (varname) of type t or ptrTo(t) if isptr==true.
...
func (x *genRunner) dec(varname string, t reflect.Type, isptr bool) {
	...
	case reflect.Struct:
		if inlist {
			// no need to create temp 01-variable if isptr, or x.F or x[F]
